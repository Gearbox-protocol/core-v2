...
contract UniversalAdapterTest is BalanceHelper, CreditFacadeTestHelper {
    ...
    function setUp() public {
        ...
        creditConfigurator.allowContract(
            address(targetMock1),
            address(adapterMock1)
        );
        creditConfigurator.allowContract(
            address(targetMock2),
            address(adapterMock2)
        );
        creditConfigurator.allowContract(
            UNIVERSAL_CONTRACT,
    ...
    function test_UA_01_constructor_sets_correct_values() public {
        assertEq(
            address(universalAdapter.creditManager()),
            address(creditManager),
            "Incorrect credit manager address"
        );
        assertEq(
            universalAdapter.targetContract(),
            UNIVERSAL_CONTRACT,
            "Incorrect target contract address"
        );
    }

    /// @notice [UA-2]: UniversalAdapter `revokeAllowances` reverts if called not from multicall
    function test_UA_02_revokeAllowances_reverts_if_called_not_from_multicall()
        public
    {
        evm.prank(USER);
    ...
    function test_UA_03_revokeAllowances_reverts_if_passed_zero_address()
        public
    {
            ...
            multicallBuilder(
                MultiCall({
                    ...
                    callData: abi.encodeWithSelector(
                        universalAdapter.revokeAdapterAllowances.selector,
                        revocations
                    )
                })
            )
    ...
    function test_UA_04_revokeAllowances_works_as_expected() public {
        ...
        creditFacade.multicall(
            multicallBuilder(
                MultiCall({
                    ...
                    callData: abi.encodeWithSelector(
                        adapterMock1.approveToken.selector,
                        usdc,
                        10
                    )
                }),
                ...
                MultiCall({
                    ...
                    callData: abi.encodeWithSelector(
                        adapterMock2.approveToken.selector,
                        dai,
                        30
                    )
                })
        ...
        revocations[1] = RevocationPair({
            spender: address(targetMock2),
            token: usdc
        });
        revocations[2] = RevocationPair({
            spender: address(targetMock2),
            token: dai
        });
