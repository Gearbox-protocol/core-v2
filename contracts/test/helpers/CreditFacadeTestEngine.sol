...
contract CreditFacadeTestEngine is DSTest {
    ...
    function _openTestCreditAccount()
        internal
    ...
    function _openExtraTestCreditAccount()
        internal
        returns (address creditAccount, uint256 balance)
    {
        uint256 accountAmount = cft.creditAccountAmount();
    ...
    function _closeTestCreditAccount() internal {
        ...
        (, , uint256 underlyingToClose) = creditManager
            .calcCreditAccountAccruedInterest(creditAccount);
        uint256 underlyingBalance = cft.tokenTestSuite().balanceOf(
            underlying,
            creditAccount
        );

        if (underlyingToClose > underlyingBalance) {
            cft.tokenTestSuite().mint(
                underlying,
                USER,
                underlyingToClose - underlyingBalance
            );

            cft.tokenTestSuite().approve(
                underlying,
                USER,
    ...
    function expectTokenIsEnabled(
        ...
        bool state = creditManager.tokenMasksMap(token) &
    ...
    function executeOneLineMulticall(
        ...
        creditFacade.multicall(
            multicallBuilder(
                MultiCall({ target: targetContract, callData: callData })
            )
        );
    }
    ...
    function expectSafeAllowance(address target) internal {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            assertLe(
                IERC20(token).allowance(creditAccount, target),
                1,
