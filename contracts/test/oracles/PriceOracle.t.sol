    ...
    IPriceOracleV2Exceptions
{
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            assertEq(
                ...
                string(
                    abi.encodePacked(
                        "Incorrect pricefeed ",
                        tokenTestSuite.symbols(tokensAdded[i])
                    )
                )
            );
        }
    ...
    function test_PO_02_addPriceFeed_reverts_for_zero_address_and_incorrect_contracts()
        public
    {
        evm.startPrank(CONFIGURATOR);
    ...
    function test_PO_03_addPriceFeed_adds_pricefeed_and_emits_event() public {
        for (uint256 sc = 0; sc < 2; sc++) {
            ...
            priceFeed.setSkipPriceCheck(
                skipCheck ? FlagState.TRUE : FlagState.FALSE
            );
            ...
            (address newPriceFeed, bool sc_flag, uint256 decimals) = priceOracle
                .priceFeedsWithFlags(address(token));

            assertEq(newPriceFeed, address(priceFeed), "Incorrect pricefeed");

            assertEq(
                priceOracle.priceFeeds(address(token)),
                address(priceFeed),
                "Incorrect pricefeed"
            );
    ...
    function test_PO_04_getPrice_reverts_if_depends_on_address_but_zero_address_was_provided()
        public
    {
        ERC20Mock token = new ERC20Mock("Token", "Token", 17);
    ...
    function test_PO_05_getPrice_reverts_if_not_passed_skipCheck_when_its_enabled()
        public
    {
        for (uint256 sc = 0; sc < 2; sc++) {
            ...
            priceFeed.setSkipPriceCheck(
                skipForCheck ? FlagState.TRUE : FlagState.FALSE
            );
    ...
    function test_PO_07_convertFromUSD_and_convertToUSD_computes_correctly(
        uint128 amount
    ) public {
    ...
    function test_PO_09_fastCheck_computes_correctly() public {
        (uint256 collateralWETH, uint256 collateralUSDC) = priceOracle
            .fastCheck(
                5 * WAD,
                tokenTestSuite.addressOf(Tokens.WETH),
                10 * 10**6,
        ...
        assertEq(
            collateralWETH,
            5 * DAI_WETH_RATE * 10**8,
            "Incorrect collateral WETH"
        );
